...
class user_profile (ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['request_count'] = friend_request.objects.filter(
            friend_user_name=self.request.user.username).aggregate(sum_all=Sum('request_count')).get('sum_all')
        context['friend_count'] = friend_list.objects.filter(Q(request_user_name=self.request.user.username) | Q(
            username=self.request.user.username)).aggregate(sum_all=Sum('friend_count')).get('sum_all')
        return context
...
def about_form(request):
    if request.method == "POST":
        ...
        if profile.objects.filter(username=username).exists():
            ...
            profile_database = profile(name=name, username=username, skills=skills,
                                       framework=framework, education=education, phone=phone, email=email, description=description, gender=gender)
...
class community_full (DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['status'] = post.objects.all()
        context['comment'] = user_comment.objects.all()
        context['comment_count'] = user_comment.objects.filter(
            post_id=self.kwargs['pk']).aggregate(sum_all=Sum('comment_count')).get('sum_all')
        return context
...
class user_friend_request (ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(user_friend_request,
...
class user_friend_list (ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(user_friend_list, self).get_context_data(
            *args, **kwargs)
        context['status'] = post.objects.all()
        context['comment'] = user_comment.objects.all()
        context['profile_list'] = profile.objects.all()
...
def chat_form(request):
    if request.method == "POST":
        ...
        if message == "":
            ...
            chat_database = chat(username=username, name=name,
                                 chat_user_name=chat_user_name, chat_name=chat_name, message=message)
            chat_database.save()
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
def search_e(request):
    if request.method == 'POST':
        ...
        comment = user_comment.objects.all()
        return render(request, 'search.html', {'search': search, 'post_list': post_list, 'comment': comment})
    else:
        return render(request, "search.html")
